	...
	def draw(A):
		...
		for B in pygame.event.get():
			...
			if B.type==pygame.KEYDOWN:
				if B.key==pygame.K_q:A.tetris.button=ROATE_LEFT_BTN
				if B.key==pygame.K_e:A.tetris.button=ROATE_RIGHT_BTN
				if B.key==pygame.K_a:A.tetris.button=LEFT_BTN
				if B.key==pygame.K_s:A.tetris.button=BOTTOM_BTN
				if B.key==pygame.K_d:A.tetris.button=RIGHT_BTN
				if B.key==pygame.K_c:A.tetris.button=HOLD_BTN
		...
		for F in range(len(A.tetris.next_pieces)):D=pygame.transform.scale(preview[A.tetris.next_pieces[F]],({1:SCALE*1,2:SCALE*2,3:SCALE*2,4:SCALE*2,5:SCALE*2,6:SCALE*2,7:SCALE*2}[A.tetris.next_pieces[F]],{1:SCALE*4,2:SCALE*3,3:SCALE*3,4:SCALE*3,5:SCALE*3,6:SCALE*3,7:SCALE*2}[A.tetris.next_pieces[F]]));A.screen.blit(D,D.get_rect(topleft=(SCALE*10+X_OFFSET_BRICKS,Y_OFFSET_BRICKS+4*SCALE*F)))
...
class Bot:
	...
	def get_current_piece(self):
		...
		if self.current == 0:
			self.current = self.main[5][ROWS-2]
